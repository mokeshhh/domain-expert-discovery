...
function Notification({ message, type = 'success', onClose }) {
  ...
  return (
    <div
      className={`notification-toast ${type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}
      role="alert"
    >
      {message}
    </div>
...
export default function ExpertDetail() {
  ...
  // Retrieve saved experts state on navigation back
  const expertsState = location.state?.expertsState;
  ...
  }, [id, isLoggedIn, userEmail, API_URL]);
  ...
  const handleBack = () => {
    navigate('/experts', { state: expertsState || {} });
  };
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-8 md:p-10 relative text-gray-900" style={{ marginTop: 20 }}>
        <button
          onClick={handleBack}
          className="text-blue-500 hover:underline font-medium mb-8 inline-block bg-transparent border-none cursor-pointer"
        >
          ← Back to experts
        </button>
        ...
        <div className="space-y-2 mb-5">
          ...
          {expert.linkedin_url && (
            <p>
              ...
              <a href={expert.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="#2563eb" strokeWidth="1.5" viewBox="0 0 24 24">
          ...
          {expert.profile_url && (
            <p>
              ...
              <a href={expert.profile_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="#2563eb" strokeWidth="1.5" viewBox="0 0 24 24">
        ...
        {wikiInfo && (
          <div className="bg-gray-50 p-4 rounded-lg mb-6 flex gap-4 items-start">
            {wikiInfo.thumbnail && (
              <img src={wikiInfo.thumbnail.source} alt={`${expert.name} thumbnail`} className="w-24 h-24 rounded shadow-md object-cover" />
            )}
            <div>
              {wikiInfo.description && <p className="mb-1 font-semibold text-gray-800"><strong>Occupation:</strong> {wikiInfo.description}</p>}
              {wikiInfo.extract && <p className="text-gray-700">{wikiInfo.extract}</p>}
            </div>
          </div>
        )}
        ...
        {expert.wikipedia_url && (
          <p className="mt-4">
            <a href={expert.wikipedia_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline font-medium inline-flex items-center gap-1">
